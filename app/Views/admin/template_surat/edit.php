      ...
      <div class="card-body ml-5" style="margin-right:2.2cm">
         ...
         <form>
            ...
            <div class="form-group">
               ...
               <input type="text" id="nama-template" class="form-control" value="<?= $template['nama_template'] ?>">
            </div>
            ...
            <button type="button" id="submit" class="btn btn-md btn-success btn-block">Ubah Template</button>
         </form>
         <!-- ==== end form ==== -->
      </div>
   </div>
...
<script>
   ...
   DecoupledEditor
      ...
      document.querySelector('#submit').addEventListener('click', () => {
         const id_template = document.getElementById('id-template')
         const nama = document.getElementById('nama-template')
         const old_nama = document.getElementById('old-nama-template')
         ...
         axios.get('/admin/template/all')
            .then(function (response) {
               ...
               const filtered_template = templates.filter(element => {
                  return element.nama_template.toLowerCase() != old_nama.value.toLowerCase()
               })

               // cek nama template
               let template_exist = false

               filtered_template.forEach(element => {
                  if(element.nama_template.toLowerCase().replace(/\s+/g, '') == nama.value.toLowerCase().replace(/\s+/g, '')) template_exist = true;
               });

               if(template_exist) return alert('Nama template telah digunakan')
               const data = {
                  <?= csrf_token() ?>: '<?= csrf_hash() ?>',
                  id: id_template.value,
                  nama_template: nama.value,
                  template,
               }
               axios.put('<?= base_url('admin/template/update') ?>', data)
                  .then(function (response) {
                     Swal.fire({
                        position: 'center',
                        icon: 'success',
